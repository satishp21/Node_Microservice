...
export const DeliverySignUp = async (
  ...
  const userPassword = await genHash(password);
  ...
  if (result) {
    ...
    const signature = await genToken({
...
export const DeliveryLogin = async (
  ...
  if (deliveryUser) {
    const validation = await passCheck(password, deliveryUser.password);

    if (validation) {
      const signature = genToken({
        id: String(deliveryUser._id),
...
export const GetDeliveryProfile = async (
  ...
  const deliveryUser = res.locals.user;
...
export const EditDeliveryProfile = async (
  ...
  const customerInputs = plainToClass(CreateDeliveryUserInput, req.body);
