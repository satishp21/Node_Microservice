...
export const createVandor = async(req : Request, res : Response)=> {
    ...
    const {name , ownerName, address, pincode, password, email, foodType} = <createVandorInput>req.body

    const hashedpass = await genHash(password)

    const createVandor = await Vandor.create({name , ownerName, address, pincode, password : hashedpass, email})
    return res.json({data : createVandor, message : 'vandor created succesfully'})
